...
Definition sorted' (al : list nat) := forall i j iv jv,
    i < j -> nth_error al i = Some iv -> nth_error al j = Some jv -> iv <= jv.
...
Lemma if_sorted'_sorted_eq (l : list nat) : sorted' l -> sorted l.
Proof.
  induction l.
  * constructor.
...
Lemma if_sorted_sorted'_eq (l : list nat) : sorted l -> sorted' l.
Proof.
...
Theorem sorted_eq_sorted' (l : list nat) : sorted l <-> sorted' l.
Proof. split. apply if_sorted_sorted'_eq. apply if_sorted'_sorted_eq. Qed.

Theorem sorted'_eq_bool (l : list nat) : sorted' l <-> sorted_bool l = true.
Proof. split. 
  intros. apply sorted_bool_eq. apply sorted_eq_sorted'. auto.
  intros. apply sorted_eq_sorted'. apply sorted_bool_eq. auto.
Qed.

Instance sorted'_dec (l : list nat) : Dec (sorted' l).
Proof.
  dec_eq. assert (P: sorted' l <-> sorted_bool l = true). apply sorted'_eq_bool.
  destruct (sorted_bool l). left. apply P. auto. 
  right. unfold not. intros. apply P in H. discriminate H.
Qed.
...
(* I don't know if this is decidable. *)
Instance is_sorting_alg_dec (f: list nat -> list nat) : Dec (is_a_sorting_algorithm f).
Proof. Admitted.
