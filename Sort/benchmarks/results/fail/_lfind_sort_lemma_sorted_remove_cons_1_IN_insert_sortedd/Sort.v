...
Proof.
    ...
    * intros. unfold insert. 
    ...
    lfind_debug.
    Admitted.
    (* apply sortedd_remove_cons. split. apply leb_correct. auto. apply sortedd_single.
    -- assert (Q: sortedd (a :: n :: l)). auto. unfold sortedd in Q.
        assert (R: 0 < 1). auto. apply Q with (iv:=a) (jv:=n) in R.
        unfold insert. unfold insert in H. bdestruct (a0 <=? n). apply sortedd_remove_cons. split. apply leb_correct. auto.
        auto. apply leb_correct in H0. apply leb_complete_conv in H1. unfold gt in H2. 
         apply sortedd_remove_cons. split. apply leb_correct. auto. apply sortedd_remove in H. auto.
         fold insert. apply sortedd_remove_cons. split. apply leb_correct. auto. apply sortedd_remove in H.
    ...
    apply leb_correct in H0. rewrite H0. apply sortedd_remove_cons. split. auto. auto.
Qed. *)
