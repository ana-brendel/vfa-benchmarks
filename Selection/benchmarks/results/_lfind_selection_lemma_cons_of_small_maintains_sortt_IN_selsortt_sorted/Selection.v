...
Lemma cons_of_small_maintains_sort: forall bl y n,
  n = list_length bl -> y <=* bl -> sorted (selsort bl n) -> sorted (y :: selsort bl n).
Proof.
...
Lemma selsort_sorted : forall n al, list_length al = n -> sorted (selsort al n).
Proof.
...
Function selsortt l {measure list_length l} :=
  match l with
  | [] => []
...
Lemma selsortt_perm : forall n l, list_length l = n -> Permutation l (selsortt l).
Proof.
  induction n.
...
Lemma list_has_length {T} (l : list T): exists x, list_length l = x.
...
Proof. 
  intros. assert (L: exists x, list_length bl = x). apply list_has_length.
...
Lemma selsortt_sorted : forall n al, list_length al = n -> sorted (selsortt al).
Proof. 
  intros. generalize dependent al. induction n.
  - intros. assert (P : al = []). destruct al. auto. simpl in H. inversion H. 
  rewrite P. simpl. apply sorted_nil.
  - intros. destruct al. simpl in H. inversion H.
  simpl in H. inversion H. simpl. pose (select_exists al n0).
  inversion e. inversion H0. rewrite selsortt_equation. rewrite H2. 
  lfind_debug.
  Admitted.
  
  (* apply cons_of_small_maintains_sortt.
  + eapply select_smallest. eauto.
