...
Theorem t_update_permute {T} `{ _ : Dec_Eq T} (n : nat) (v1 v2 : T) (x1 x2 : nat) (m : total_map T) :
  x2 <> x1 -> TotalMapEqual n (t_update (t_update m x2 v2) x1 v1) (t_update (t_update m x1 v1) x2 v2).
Proof.
  destruct m. unfold TotalMapEqual. intros. unfold t_update. unfold TotalMapEqualHelper. intros. simpl. induction l.
  - simpl. destruct (eq_nat_dec x1 x2). rewrite e in H0. contradiction. replace (x2 =? x1) with false. replace (x1 =? x2) with false.
    simpl. destruct (eq_nat_dec x2 n). destruct (eq_nat_dec x1 n). rewrite e in H0; rewrite e0 in H0. contradiction.
    rewrite e. replace (n =? n) with true. replace (x1 =? n) with false. reflexivity. symmetry. apply Nat.eqb_neq. auto. apply beq_nat_refl.
    replace (x2 =? n) with false. reflexivity. symmetry. apply Nat.eqb_neq. auto. symmetry. apply Nat.eqb_neq. auto. symmetry. apply Nat.eqb_neq. auto.
  - destruct a. destruct (eq_nat_dec x1 n0).
    * simpl. destruct (eq_nat_dec x2 n0). rewrite e in H0. rewrite e0 in H0. contradiction. replace (n0 =? x2) with false. rewrite e. replace (n0 =? n0) with true.
      simpl. replace (n0 =? x2) with false. replace (n0 =? n0) with true. reflexivity. apply beq_nat_refl. symmetry. apply Nat.eqb_neq. auto. 
      apply beq_nat_refl. symmetry. apply Nat.eqb_neq. auto.
    * simpl. replace (n0 =? x1) with false. destruct (eq_nat_dec x2 n0). 
      replace (n0 =? x2) with true. simpl. replace (n0 =? x1) with false. replace (n0 =? x2) with true. reflexivity. rewrite e; apply beq_nat_refl.
      symmetry. apply Nat.eqb_neq. auto. rewrite e. apply beq_nat_refl. 
      replace (n0 =? x2) with false. simpl. replace (n0 =? x1) with false. replace (n0 =? x2) with false. simpl. destruct (n0 =? n).
      reflexivity. apply IHl.
      symmetry; apply Nat.eqb_neq; auto. symmetry; apply Nat.eqb_neq; auto. symmetry; apply Nat.eqb_neq; auto. symmetry; apply Nat.eqb_neq; auto.
Qed.
...
Theorem update_same {X} `{ _ : Dec_Eq X} (n : nat) (v : X) (x : nat) (m : partial_map X): find m x = Some v -> TotalMapEqual n (update m x v) m.
Proof.
  intros. unfold update. rewrite <- H0.
  destruct m. simpl. unfold TotalMapEqual. unfold TotalMapEqualHelper. intros. simpl. induction l.
  - simpl. destruct (x =? n). reflexivity. reflexivity.
  - destruct a. simpl. destruct (eq_nat_dec n0 x).
    * rewrite e. replace (x =? x) with true. simpl. reflexivity. apply beq_nat_refl.
    * replace (n0 =? x) with false. simpl. destruct (eq_nat_dec n0 n). rewrite e; replace (n =? n) with true. reflexivity. apply beq_nat_refl.
      replace (n0 =? n) with false. apply IHl. inversion H0. replace (n0 =? x) with false. unfold find. reflexivity. 
